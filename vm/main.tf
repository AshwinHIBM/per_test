...
data "ignition_file" "bootstrap_ignition" {
  overwrite = true
  mode      = "420" // 0644
  path      = "/etc/hostname"
  source {
    source = "https://raw.githubusercontent.com/AshwinHIBM/bootstrap_ignition/main/bootstrap.ign"
  }
}

data "ignition_config" "bootstrap_ignition" {
  users = [data.ignition_user.vm_user.rendered]
  files = [data.ignition_file.bootstrap_ignition.rendered]
}
...
resource "ibm_pi_instance" "cluster_node" {
  ...
  pi_user_data = base64encode(templatefile("${path.module}/ignition/master.ign", {
    HOSTNAME    = "localhost"
    PORT        = "8080"
    OBJECT_NAME = "master.ign"
    IAM_TOKEN   = data.ibm_iam_auth_token.iam_token.iam_access_token
  }))
}
